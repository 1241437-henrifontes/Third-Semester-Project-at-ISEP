DECLARE
    p_rsmId1 RollingStockModel.id%TYPE := 12;
    p_rsmId2 RollingStockModel.id%TYPE := 13;

    p_rsmModel1 RollingStockModel.model%TYPE := 'Non Existent Model 1';
    p_rsmModel2 RollingStockModel.model%TYPE := 'Non Existent Model 2';

    p_rsmNumBogies1 RollingStockModel.numBogies%TYPE := 3;
    p_rsmNumBogies2 RollingStockModel.numBogies%TYPE := 2;

    p_rsmMaxSpeed1 RollingStockModel.maxSpeed%TYPE := 120;
    p_rsmMaxSpeed2 RollingStockModel.maxSpeed%TYPE := 320;

    p_rsmBogies1 RollingStockModel.bogies%TYPE := 'Simple';
    p_rsmBogies2 RollingStockModel.bogies%TYPE := 'Double';

    p_rsmRSDId1 RollingStockModel.RollingStockDimensionId%TYPE := 12;
    p_rsmRSDId2 RollingStockModel.RollingStockDimensionId%TYPE := 11;

    p_rsmManId1 RollingStockModel.manufacturerID%TYPE := 8;
    p_rsmManId2 RollingStockModel.manufacturerID%TYPE := 9;

    p_rsmMultipleGauges1 RollingStockModel.multipleGauges%TYPE := 'Y';
    p_rsmMultipleGauges2 RollingStockModel.multipleGauges%TYPE := 'N';

    p_locomotiveModelId1 LocomotiveModel.locomotiveModelID%TYPE := 4;
    p_locomotiveModelId2 LocomotiveModel.locomotiveModelID%TYPE := 5;

    p_locomotiveModelPower1 LocomotiveModel.power%TYPE := 6000;
    p_locomotiveModelPower2 LocomotiveModel.power%TYPE := 7000;

    p_locomotiveModelOS1 LocomotiveModel.operationalSpeed%TYPE := 60;
    p_locomotiveModelOS2 LocomotiveModel.operationalSpeed%TYPE := 70;

    p_locomotiveModelTraction1 LocomotiveModel.traction%TYPE := 400;
    p_locomotiveModelTraction2 LocomotiveModel.traction%TYPE := 500;
